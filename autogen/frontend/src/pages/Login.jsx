...
export default function Login() {
  ...
  const { isAuthenticated, hasProfile, loading, refreshAuthState } = useAuth();
  ...
  useEffect(() => {
    if (loading) return;
    if (isAuthenticated && hasProfile) navigate("/app", { replace: true });
  ...
  const requestOtp = async (event) => {
    ...
    try {
      ...
      setStep("otp");
      setNotice("OTP sent through backend Twilio Verify service.");
    } catch (err) {
  ...
  const startGoogleOAuth = async () => {
    setError("Google OAuth is disabled for this deployment. Use phone OTP.");
  };
        ...
        <div className="grid w-full gap-8 lg:grid-cols-[1.15fr_0.85fr]">
          <section className="rounded-[32px] border border-white/60 bg-white/85 p-8 shadow-[0_30px_100px_rgba(0,0,0,0.12)] backdrop-blur-xl md:p-12">
            ...
            <p className="mt-4 max-w-2xl text-base font-medium text-on-surface-variant md:text-lg">
              Sign in with Twilio OTP, then finish your coastal profile so the backend can calculate the safest fishing advice for your zone and language.
            </p>
          ...
          <section className="rounded-[32px] border border-white/60 bg-white/90 p-6 shadow-[0_30px_100px_rgba(0,0,0,0.14)] backdrop-blur-xl md:p-8">
            <div className="mb-6 flex items-start justify-between gap-4">
              ...
              <button
                ...
                className="rounded-full bg-surface-container-highest px-4 py-2 text-sm font-black opacity-70"
              >
                Phone OTP Only
              </button>
            </div>
