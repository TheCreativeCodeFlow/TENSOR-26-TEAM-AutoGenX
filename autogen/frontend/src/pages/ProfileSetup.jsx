...
export default function ProfileSetup() {
  ...
  useEffect(() => {
    const loadZones = async () => {
      try {
        ...
        setZones(list);
        if (!list.length) {
          setError("No coastal zones are configured yet. You can continue by entering your coastal area manually.");
        }
      } catch (_error) {
        setZones([]);
        setError("Could not load coastal zones. Enter coastal area manually and continue.");
      }
    };
    loadZones();
  }, []);
  ...
  const quickPickZones = zones.slice(0, 12);
  ...
  const submit = async (event) => {
    ...
    setSuccess("");
    try {
      const selectedZone = zones.find((zone) => zone.id === formData.primaryZoneId);
      const latitude = Number(selectedZone?.center?.lat ?? formData.latitude);
      const longitude = Number(selectedZone?.center?.lng ?? formData.longitude);
      const phone = String(formData.phone || "").trim();
      const locationLabel = selectedZone?.name || String(formData.locationLabel || "").trim();

      if (!formData.primaryZoneId) {
        throw new Error("Please select a coastal area.");
      }
      if (!phone) {
        throw new Error("Safety phone is required. Sign in with phone OTP or add a valid phone number.");
      }
      if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) {
        throw new Error("Selected coastal area is missing valid coordinates.");
      }
      if (!locationLabel) {
        throw new Error("Could not resolve location from selected coastal area.");
      }
        ...
        <form onSubmit={submit} className="slide-up space-y-5 rounded-[28px] border border-white/70 bg-white/90 p-5 shadow-[0_30px_100px_rgba(0,0,0,0.12)] backdrop-blur-xl md:p-8">
          ...
          <section className="grid gap-4 md:grid-cols-2">
            ...
            <label className="block">
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  update("locationLabel", zone?.name || "");
                  update("latitude", zone?.center?.lat ?? "");
                  update("longitude", zone?.center?.lng ?? "");
                }}
                className="w-full rounded-2xl border border-surface-container-highest bg-white px-4 py-4 text-lg font-bold outline-none transition focus:border-primary focus:ring-4 focus:ring-primary/10"
                required
              >
            ...
            <label className="block">
              ...
              />
              {user?.phone ? <p className="mt-2 text-xs font-medium text-outline">Verified through Twilio phone OTP login.</p> : null}
            </label>

            <input type="hidden" value={formData.locationLabel} readOnly />
