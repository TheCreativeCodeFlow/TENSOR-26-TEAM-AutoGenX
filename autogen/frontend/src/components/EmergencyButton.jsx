...
const SOS_QUEUE_KEY = "offline_sos_queue_v1";
...
export default function EmergencyButton({ compact = false, className = "" }) {
  ...
  useEffect(() => {
    const flushQueue = async () => {
      ...
      for (const item of pending) {
        try {
          await emergencyApi.sos(item.payload);
        } catch (_error) {
          failed.push(item);
        }
      }
      writeQueue(failed);
      if (!failed.length) {
        setMessage("Queued SOS delivered after network restore.");
      }
    };
  ...
  const trigger = async () => {
    ...
    try {
      ...
      writeQueue(queue);
      setMessage("No network. SOS saved offline and will auto-send when signal returns.");
      window.location.href = "tel:1554";
    } finally {
      setBusy(false);
    }
